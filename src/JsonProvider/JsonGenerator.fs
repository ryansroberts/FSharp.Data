...
module JsonTypeBuilder = 
  ...
  and internal generateJsonType ctx = function
    ...
    | InferedType.Collection (SingletonMap(_, (_, typ))) -> 
        ...
        let convTyp, convFunc = ReflectionHelpers.makeFuncDelegate elementConv ctx.Representation
        // Build a function `packer = fun x -> %%(ctx.Packer x)`
        let _, packFunc = ReflectionHelpers.makeFuncDelegate ctx.Packer (ctx.Replacer.ToRuntime typeof<JsonValue>)
        ...
        for prop in props do
          ...
          let propTy, getter =
            if not prop.Optional || (prop.Optional && prop.Type = Primitive(typeof<string>, None)) then 
              ...
              let convTyp, convFunc = ReflectionHelpers.makeFuncDelegate valConv ctx.Representation
              let _, packFunc = ReflectionHelpers.makeFuncDelegate ctx.Packer (ctx.Replacer.ToRuntime typeof<JsonValue>)
